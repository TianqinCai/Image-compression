...
// END PA 4 FUNCTIONS
  ...
  private:
	...
	void buildTreehelper(PNG const& source, QuadtreeNode * temp, int res);


	void copytree(QuadtreeNode* ohtertemp, QuadtreeNode * temp, int res);

	RGBAPixel getPixelhelper(QuadtreeNode* temp, int x, int y) const;

	void deletetree(QuadtreeNode * temp);

	void decompresshelper(PNG* result, QuadtreeNode * temp, int res) const;

	void Rotatehelper(QuadtreeNode* temp, Quadtree const& other, int res);

	void Rotatehelper1(QuadtreeNode* temp, QuadtreeNode* othertemp, int res);

	void prunehelper2(QuadtreeNode* temp, int tolerance);

	void prunehelper(QuadtreeNode* temp, QuadtreeNode* temp2, int tolerance, int &flag);

	void calculateLeaf(QuadtreeNode* temp, int &result) const;

	int pruneSizeonthis(int tolerance);

	int findIdeal(int &result, int numLeaves, int x,int& num);

	int findIdeal2(int min, int max, int numLeaves,int &result);

	//void NWRotatehelper(QuadtreeNode* temp);

	//void init(PNG const& source, QuadtreeNode* temp, int resolution);

	//void unvisited(QuadtreeNode* temp);
